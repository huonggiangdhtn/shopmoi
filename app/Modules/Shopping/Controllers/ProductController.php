...
use App\Modules\Shopping\Models\Product;
use App\Modules\Shopping\Models\Category;
use App\Modules\Shopping\Models\Brand;
use App\Modules\Shopping\Models\Productextend;
use App\Modules\Shopping\Models\GroupPrice;
...
class ProductController extends Controller
{
    ...
    public function index()
    {
        ...
        $cat_id = 0;
        return view('Shopping::products.index', compact('products', 'breadcrumb', 'active_menu', 'cats', 'cat_id'));
    }
    ...
    public function productSort(Request $request)
    {
        ...
        $breadcrumb = '
        <li class="breadcrumb-item"><a href="#">/</a></li>
        <li class="breadcrumb-item  " aria-current="page"><a href="' . route('admin.product.index') . '">hàng hóa</a></li>
        ';
        $cats = Category::where('status', 'active')->get();
        return view('Shopping::products.index', compact('products', 'breadcrumb', 'searchdata', 'active_menu', 'cats', 'cat_id'));
    }
    public function productSearch(Request $request)
    {
        ...
        if ($request->datasearch) {
            ...
            return view('Shopping::products.search', compact('products', 'breadcrumb', 'searchdata', 'active_menu'));
        } else {
            return redirect()->route('admin.product.index')->with('success', 'Không có thông tin tìm kiếm!');
        }
    }
    ...
    public function create()
    {
        ...
        $tags = \App\Modules\Tag\Models\Tag::where('status', 'active')->orderBy('title', 'ASC')->get();
        return view('Shopping::products.create', compact('breadcrumb', 'active_menu', 'categories', 'brands', 'tags'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($tag_ids && count($tag_ids) > 0) {
            $tagservice = new \App\Modules\Tag\Controllers\TagController();
            $tagservice->store_product_tag($status->id, $tag_ids);
        }

        if ($status) {
            return redirect()->route('admin.product.index')->with('success', 'Tạo hàng hóa thành công!');
        } else {
    ...
    public function show(string $id)
    {
        ...
        if ($product) {
            ...
            $breadcrumb = '
            <li class="breadcrumb-item"><a href="#">/</a></li>
            <li class="breadcrumb-item  " aria-current="page"><a href="' . route('admin.product.index') . '">products</a></li>
            ...
            return view('Shopping::products.show', compact('breadcrumb', 'product', 'active_menu', 'categories', 'brands'));
        } else {
    ...
    public function edit(string $id)
    {
        ...
        if ($product) {
            ...
            return view('Shopping::products.edit', compact('breadcrumb', 'product', 'active_menu', 'categories', 'brands', 'tags', 'tag_ids'));
        } else {
            return back()->with('error', 'Không tìm thấy dữ liệu');
        }
    }
    ...
    public function productPriceView($id)
    {
        ...
        foreach ($group_prices as $gprice) {
            ...
            if ($gprice->price === null) {
                ...
                $data['price'] = 0;
                $data['product_id'] = $id;
                GroupPrice::create($data);
            }
        }
        $productextend =  Productextend::where('product_id', $id)->first();
        if (! $productextend) {
            $data['old_price'] = 0;
            $data['product_id'] = $id;
            $productextend = Productextend::create($data);
        }
        return view('Shopping::products.viewprice', compact('product', 'breadcrumb', 'active_menu', 'group_prices', 'productextend'));
    }
    public function productPriceUpdate(Request $request)
    {
        ...
        $data = $request->all();
        $productextend =  Productextend::where('product_id', $data['id'])->first();
        ...
        $gprices =  GroupPrice::where('product_id', $data['id'])->get();
        // dd($data);
        foreach ($gprices as $gprice) {
            $gprice->price = isset($data['gp' . $gprice->ugroup_id]) ? $data['gp' . $gprice->ugroup_id] : 0;
            $gprice->save();
        }
        return redirect()->route('admin.product.priceview', $data['id'])->with('success', 'cập nhật thành công');
    }
}
