...
@section('content')
    <div class="intro-y flex items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">Điều chỉnh bài viết</h2>
    </div>
            ...
            <form method="post" action="{{ route('admin.blog.update', $blog->id) }}">
                ...
                <div class="intro-y box p-5">
                    <div>
                        ...
                        <input id="title" name="title" type="text" value="{{ $blog->title }}" class="form-control"
                            placeholder="title" required>
                    </div>
                    <div class="mt-3">
                        <label class="form-label">Photo</label>
                        <div class="px-4 pb-4 mt-5 flex items-center cursor-pointer relative">
                            <div data-single="true" id="mydropzone" class="dropzone"
                    ...
                    <div class="mt-3">
                        ...
                        <textarea class="form-control" name="summary" id="editor1">{{ $blog->summary }}</textarea>
                    </div>
                    <div class="mt-3">
                        <label class="form-label">Nội dung</label>
                        <textarea class="editor" name="content" id="editor2">{{ $blog->content }}</textarea>
                    </div>
                    <div class="mt-3">
                        <div class="flex flex-col sm:flex-row items-center">
                            ...
                            <select name="cat_id" class="form-select mt-2 sm:mr-2">
                                ...
                                @foreach ($categories as $cat)
                                    <option value="{{ $cat->id }}" {{ $cat->id == $blog->cat_id ? 'selected' : '' }}>
                                        {{ $cat->title }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="mt-3">
                        ...
                        <select id="select-junk" name="tag_ids[]" multiple placeholder="..." autocomplete="off">
                            @foreach ($tags as $tag)
                                <option value="{{ $tag->id }}"
                                    {{ in_array($tag->id, $tag_ids->toArray()) ? 'selected' : '' }}>
                                    {{ $tag->title }}
                                </option>
                            @endforeach
                        </select>
